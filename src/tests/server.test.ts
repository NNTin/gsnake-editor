...
describe("Server API tests", () => {
  ...
  describe("POST /api/test-level", () => {
    it("should return success with { success: true }", async () => {
      const response = await request(app).post("/api/test-level").send(testLevelData).expect(200);
  ...
  describe("GET /api/test-level", () => {
    it("should return the stored payload", async () => {
      // First store a level
      await request(app).post("/api/test-level").send(testLevelData).expect(200);
    ...
    it("should return 404 when nothing is stored", async () => {
      ...
      const response = await request(app).get("/api/test-level");
